...
CONFIG_MAIN = SimulationConfig(
    ...
    common=CommonParams(
        ...
        seed=1000,            # 乱数シード基点（trialごとにseed+i）
    ),
    ...
    tuning=TuningParams(
        ...
        objective="mean", # mean_plus_drift
    ...
    search_spaces=SearchSpaces(
        pp=PPSearchSpace(
            ...
            # NOTE: log=True の場合 low は 0 より大きい必要がある（Optuna制約）。
            # 0 を探索したい場合は low を極小正数にするか、探索側で 0 を特別扱いする。
            mu_lambda=SearchRange(low=1e-12, high=2, log=True, type="float", step=None, choices=None),     # 緩和係数スカラー（大きいほど更新が速い）
        ...
        pp_sgd=PPSearchSpace(
            ...
            # NOTE: log=True の場合 low は 0 より大きい必要がある（Optuna制約）。
            mu_lambda=SearchRange(low=1e-12, high=2, log=True, type="float", step=None, choices=None),
