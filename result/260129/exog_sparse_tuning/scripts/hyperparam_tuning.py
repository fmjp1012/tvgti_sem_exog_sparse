...
def _suggest_from_space(
    ...
    if param_type == "float":
        ...
        if log and low <= 0.0:
            if high <= 0.0:
                raise ValueError(
                    f"{name}: log=True ですが low/high が正でありません ({low=}, {high=}). "
                    "low>0 にするか log=False にしてください。"
                )
            # Optuna は log=True で low==0 を許容しないため、極小正数へクランプする。
            # 0 を厳密に探索したい場合は「0を含むカテゴリ + 正のlog空間」のように別設計が必要。
            eps = max(1e-12, high * 1e-12)
            if eps >= high:
                eps = high / 10.0
            low = eps
