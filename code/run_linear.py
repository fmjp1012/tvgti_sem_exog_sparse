...
def parse_args() -> Optional[Path]:
    ...
    parser = argparse.ArgumentParser(
        description="Linearシナリオの実験実行（設定は config.py で変更）"
    )
    parser.add_argument(
        "--hyperparam_json",
        type=Path,
...
def main() -> None:
    ...
    def run_trial(trial_seed: int):
        ...
        if run_pc_flag:
            ...
            pc = PCSEM(
                ...
                show_progress=False, name="pc_baseline", T_init=T_mat,
            )
        ...
        if run_co_flag:
            ...
            co = PCSEM(
                ...
                show_progress=False, name="co_baseline", T_init=T_mat,
            )
        ...
        if run_sgd_flag:
            ...
            sgd = PCSEM(
                ...
                show_progress=False, name="sgd_baseline", T_init=T_mat,
            )
    ...
    filename = make_result_filename(
        ...
        params={
            ...
            "maxweight": max_weight, "stde": std_e,
